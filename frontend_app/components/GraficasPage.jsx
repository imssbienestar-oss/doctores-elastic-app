...
const styles = {
  ...
  headerTitle: { textAlign: "center", marginBottom: "30px", fontSize: "45px", color: "#333", fontWeight: "600" },
  sectionTitle: { textAlign: "center", marginTop: "50px", marginBottom: "25px", fontSize: "24px", color: "#333", fontWeight: "600", borderTop: "1px solid #eee", paddingTop: "30px" },
  chartsGridContainer: { display: "grid", gridTemplateColumns: "1fr 1.5fr", gap: "25px", width: "100%", maxWidth: "1200px", margin: "0 auto 40px auto" },
  pieChartsColumn: { display: "flex", flexDirection: "column", gap: "25px" },
  chartWrapper: { height: "450px", width: "100%", background: "#f9f9f9", padding: "20px", borderRadius: "8px", boxSizing: "border-box", display: "flex", flexDirection: "column" },
  chartTitle: { textAlign: "center", color: "#333", fontSize: "18px", fontWeight: "500", marginBottom: "5px", flexShrink: 0 },
  tableContainer: { width: "100%", maxWidth: "1000px", margin: "0 auto", background: "#f9f9f9", padding: "20px", borderRadius: "8px", boxShadow: "0 4px 12px rgba(0,0,0,0.1)" },
  dataTable: { width: "100%", borderCollapse: "collapse", fontSize: "0.9em" },
  dataTableTh: { backgroundColor: "#006657", color: "white", padding: "10px 12px", textAlign: "center", border: "1px solid #005c4e" },
  dataTableTd: { padding: "10px 12px", border: "1px solid #ddd", color: "#333", textAlign: "center" },
  dataTableTrEven: { backgroundColor: "#f2f2f2" },
  statisticFiltersContainer: { display: "flex", gap: "15px", marginBottom: "20px", padding: "15px", backgroundColor: "#e9ecef", borderRadius: "6px", flexWrap: "wrap", justifyContent: "center" },
  filterGroup: { display: "flex", flexDirection: "column" },
  filterLabel: { fontSize: "0.85em", color: "#495057", marginBottom: "5px", fontWeight: "500" },
  filterSelect: { padding: "8px 10px", fontSize: "0.9em", border: "1px solid #ced4da", borderRadius: "4px", minWidth: "180px", backgroundColor: "white" },
  loadingOrErrorStyle: { textAlign: "center", marginTop: "50px", fontSize: "1.2em" },
  errorStyle: { color: "red" },
  message: { textAlign: "center", padding: "20px", fontSize: "1.1em", color: "#777" },
  paginationControls: { display: "flex", justifyContent: "space-between", alignItems: "center", marginTop: "20px", padding: "10px", backgroundColor: "#f0f0f0", borderRadius: "4px" },
...
function GraficasPage() {
  ...
  useEffect(() => {
    const fetchInitialData = async () => {
      ...
      try {
        ...
        if (!estadoRes.ok || !estatusRes.ok || !nivelRes.ok) throw new Error("Error al cargar datos iniciales.");

        const estadoData = await estadoRes.json();
        const estatusData = await estatusRes.json();
        const nivelData = await nivelRes.json();

        setDataPorEstado(estadoData.map(item => ({ id: item.label, label: item.label, value: item.value })).reverse());
        setDataPorEstatus(estatusData.map(item => ({ id: item.label, label: item.label, value: item.value })));
        setDataPorNivelAtencion(nivelData.map(item => ({ id: item.label, label: item.label, value: item.value })));
  ...
  useEffect(() => {
    const fetchFilterOptions = async () => {
      ...
      try {
        const response = await fetch(`${API_BASE_URL}/api/opciones/filtros?${params.toString()}`, {
  ...
  }, [filtroEntidad, filtroUnidad, filtroEspecialidad, filtroNivel,debouncedBusqueda, token]);
  ...
  useEffect(() => {
    const fetchEstadisticaData = async () => {
      ...
      try {
        const response = await fetch(`${API_BASE_URL}/api/graficas/estadistica_doctores_agrupados?${params.toString()}`, {
          headers: { Authorization: `Bearer ${token}` }
        });
  ...
  }, [currentPageEstadistica, filtroEntidad, filtroUnidad, filtroEspecialidad, filtroNivel, debouncedBusqueda, token]);
  ...
  const handleVisualizarClick = async () => {
    ...
    if (debouncedBusqueda) params.append("search", debouncedBusqueda);
    try {
      const response = await fetch(`${API_BASE_URL}/api/doctores/detalles_filtrados?${params.toString()}`, {
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  if (error) {
    return <p style={{ ...styles.loadingOrErrorStyle, ...styles.errorStyle }}>{error}</p>;
  }
      ...
      <div style={styles.chartsGridContainer}>
        <div style={styles.pieChartsColumn}>
          <div style={styles.chartWrapper}>
            <h2 style={styles.chartTitle}>Médicos por Estatus de Cooperación</h2>
            <ResponsivePie data={dataPorEstatus} margin={{ top: 40, right: 160, bottom: 120, left: 20 }} innerRadius={0.6} padAngle={3} cornerRadius={3} activeOuterRadiusOffset={8} colors={misColoresPastel} borderWidth={1} borderColor={{ from: "color", modifiers: [["darker", 0.2]] }} enableArcLinkLabels={false} arcLabelsSkipAngle={10} arcLabelsTextColor="#ffffff" legends={[{ anchor: "right", direction: "column", justify: false, translateX: 140, translateY: 0, itemsSpacing: 5, itemWidth: 100, itemHeight: 20, itemTextColor: "#333", itemDirection: "left-to-right", itemOpacity: 1, symbolSize: 20, symbolShape: "circle" }]} />
          </div>
          <div style={styles.chartWrapper}>
            <h2 style={styles.chartTitle}>Médicos por Nivel de Atención</h2>
            <ResponsivePie data={dataPorNivelAtencion} margin={{ top: 40, right: 160, bottom: 120, left: 20 }} innerRadius={0.6} padAngle={3} cornerRadius={3} activeOuterRadiusOffset={8} colors={misColoresPastel} borderWidth={1} borderColor={{ from: "color", modifiers: [["darker", 0.2]] }} enableArcLinkLabels={false} arcLabelsSkipAngle={10} arcLabelsTextColor="#ffffff" legends={[{ anchor: "right", direction: "column", justify: false, translateX: 140, translateY: 0, itemsSpacing: 5, itemWidth: 100, itemHeight: 20, itemTextColor: "#333", itemDirection: "left-to-right", itemOpacity: 1, symbolSize: 20, symbolShape: "circle" }]} />
          </div>
        </div>
        <div style={{ ...styles.chartWrapper, height: "925px" }}>
          <h2 style={styles.chartTitle}>Médicos por Entidad Federativa</h2>
          <ResponsiveBar data={dataPorEstado} keys={["value"]} indexBy="id" layout="horizontal" margin={{ top: 10, right: 60, bottom: 100, left: 120 }} padding={0.35} valueScale={{ type: "linear" }} indexScale={{ type: "band", round: true }} colors="#BC955C" borderColor={{ from: "color", modifiers: [["darker", 1.6]] }} axisTop={null} axisRight={null} axisBottom={{ tickSize: 5, tickPadding: 5, tickRotation: 0, legend: "Número de Médicos", legendPosition: "middle", legendOffset: 50 }} axisLeft={{ tickSize: 5, tickPadding: 5, tickRotation: 0, legend: "Estado", legendPosition: "middle", legendOffset: -90 }} labelSkipWidth={12} labelSkipHeight={12} labelTextColor="white" animate={true} motionStiffness={90} motionDamping={15} />
        </div>
      </div>
      ...
      <div style={styles.statisticFiltersContainer}>
        ...
        <div style={styles.filterGroup}>
          ...
          <select id="filtroEntidad" value={filtroEntidad} onChange={handleEntidadChange} style={styles.filterSelect} disabled={isLoadingFiltros}>
            <option value="">{isLoadingFiltros ? 'Cargando...' : 'Todas las Entidades'}</option>
        ...
        <div style={styles.filterGroup}>
          ...
          <select id="filtroUnidad" value={filtroUnidad} onChange={handleUnidadChange} style={styles.filterSelect} disabled={isLoadingFiltros}>
            <option value="">{isLoadingFiltros ? 'Cargando...' : 'Todas las Unidades'}</option>
        ...
        <div style={styles.filterGroup}>
          ...
          <select id="filtroEspecialidad" value={filtroEspecialidad} onChange={handleEspecialidadChange} style={styles.filterSelect} disabled={isLoadingFiltros}>
            <option value="">{isLoadingFiltros ? 'Cargando...' : 'Todas las Especialidades'}</option>
            {opcionesEspecialidad.map(esp => (<option key={esp} value={esp}>{esp}</option>))}
          </select>
        </div>
        <div style={styles.filterGroup}>
          <label htmlFor="filtroNivel" style={styles.filterLabel}>Nivel de Atención:</label>
          <select id="filtroNivel" value={filtroNivel} onChange={handleNivelChange} style={styles.filterSelect} disabled={isLoadingFiltros}>
            <option value="">{isLoadingFiltros ? 'Cargando...' : 'Todos los Niveles'}</option>
            {opcionesNivel.map(nivel => (<option key={nivel} value={nivel}>{nivel}</option>))}
          </select>
        </div>
        {(filtroBusqueda || filtroEntidad || filtroEspecialidad || filtroNivel) && (
          <>
            <button
        ...
        <table style={styles.dataTable}>
          <thead>
            <tr>
              ...
              <th style={{ ...styles.dataTableTh, textAlign: "center" }}>Cantidad Médicos</th>
            </tr>
          </thead>
          <tbody>
            {dataEstadistica.map((item, index) => (
              <tr key={`${item.entidad}-${item.unidad}-${item.especialidad}-${item.nivel_atencion}-${index}`} style={index % 2 === 0 ? {} : styles.dataTableTrEven}>
                ...
                <td style={{ ...styles.dataTableTd, textAlign: "center" }}>{item.cantidad}</td>
              </tr>
            ))}
          </tbody>
        </table>
        <div style={{ textAlign: "right", marginTop: "10px", fontWeight: "bold", paddingRight: "10px" }}>
          Total de Médicos (con filtros aplicados): {totalDoctorsInGroups}
        </div>
        {totalPagesEstadistica > 1 && (
          <div style={styles.paginationControls}>
            <button onClick={handlePreviousPageEstadistica} disabled={currentPageEstadistica === 0 || isLoading} style={{...styles.button, ...(currentPageEstadistica === 0 || isLoading ? styles.buttonDisabled : {})}}>Anterior</button>
            <span>Página {currentPageEstadistica + 1} de {totalPagesEstadistica} (Total Grupos: {totalGroupsEstadistica})</span>
            <button onClick={handleNextPageEstadistica} disabled={currentPageEstadistica >= totalPagesEstadistica - 1 || isLoading} style={{...styles.button, ...(currentPageEstadistica >= totalPagesEstadistica - 1 || isLoading ? styles.buttonDisabled : {})}}>Siguiente</button>
          </div>
        )}
      </div>
