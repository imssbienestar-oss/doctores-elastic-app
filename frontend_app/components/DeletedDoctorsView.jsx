...
function DeletedDoctorsView({ onDoctorRestored }) {
  ...
  const proceedWithPermanentDoctorDeletion = async () => {
    ...
    if (!isPhraseMatching) {
      setPermanentDeleteModalError(
        `Texto incorrecto. Por favor, escribe exactamente: "${expectedPhrase}"`
      );
      return;
    }
    if (
      permanentDeletePinInput.length !== 4 ||
      !/^\d{4}$/.test(permanentDeletePinInput)
    ) {
  ...
  const handleSelectAllOnPage = (event) => {
    if (event.target.checked)
      setSelectedDoctorIds(deletedDoctors.map((d) => d.id_imss));
  ...
  const handleRestoreSelected = async () => {
    ...
    try {
      ...
      if (errors.length > 0) {
        setError(
          `Errores durante la restauración: ${errors.join(
            "; "
          )}. Restaurados: ${successfulRestores}.`
        );
      }
  ...
  if (!authToken && !isLoading && !currentUser)
  ...
  return (
    <div style={styles.container}>
      <div style={styles.headerContainer}>
        <h1 style={styles.title}>Registros Eliminados</h1>
              ...
              {deletedDoctors.map((doctor, index) => (
                ...
                >
                  <td style={{ ...styles.td, ...styles.checkboxCell }}>
                    <input
                      type="checkbox"
                      checked={selectedDoctorIds.includes(doctor.id_imss)}
                      onChange={() => toggleSelect(doctor.id_imss)}
                    />
                  </td>
                  <td style={styles.td}>{doctor.id_imss}</td>
                  <td style={styles.td}>
